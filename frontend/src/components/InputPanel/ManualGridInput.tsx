...
export function ManualGridInput({ matrix, onChange, n, m, p, disabled = false }: ManualGridInputProps) {
  const handleCellChange = (row: number, col: number, value: string) => {
    const numValue = parseInt(value, 10) || 1;
    const clampedValue = Math.max(1, Math.min(p, numValue));

    const newMatrix = matrix.map((r, i) => (i === row ? r.map((c, j) => (j === col ? clampedValue : c)) : r));
  ...
  return (
    <Box className='space-y-2!'>
      ...
      <Paper elevation={0} className='overflow-auto max-h-80 p-2! bg-gray-50 border border-gray-200 rounded-lg'>
        <Box
              ...
              <TextField
                ...
                sx={{
                  ...
                  '& .MuiOutlinedInput-root': {
                    backgroundColor:
                      cellValue === p ? '#fef3c7' : rowIndex === 0 && colIndex === 0 ? '#dbeafe' : 'white'
                  }
                }}
              />
            ))
