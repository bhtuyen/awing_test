...
export function MatrixDisplay({
  ...
  if (!matrix || matrix.length === 0) {
    return (
      <Paper elevation={2} className='p-4! flex items-center justify-center'>
        <Typography color='textSecondary'>Nhập dữ liệu và nhấn "Tính toán" để hiển thị ma trận</Typography>
      </Paper>
    );
  }
  ...
  const targetPosition = currentChest > 0 && currentChest <= p ? chestPositions.get(currentChest) : undefined;
  ...
  return (
    <Paper elevation={2} className='p-4!'>
      ...
      <Box className='flex flex-wrap gap-4! mb-4! text-sm'>
        ...
        <span className='flex items-center gap-1!'>🏴‍☠️ Hải tặc</span>
      </Box>

      {/* Matrix Grid */}
      <Box className='overflow-auto max-h-96'>
        <Box className='relative inline-block' sx={{ padding: '8px' }}>
          {/* Path overlay */}
          <PathLine steps={steps} currentStepIndex={currentStepIndex} cellSize={cellSize} gridGap={gridGap} />
          ...
          >
            {matrix.map((row, rowIndex) =>
              row.map((cellValue, colIndex) => {
                ...
                const isPirateHere = piratePosition.row === cellRow && piratePosition.col === cellCol;
                const isVisited = visitedCells.has(cellKey);
                const isTarget = targetPosition?.row === cellRow && targetPosition?.col === cellCol;
      ...
      <Box className='mt-4! flex flex-wrap gap-4! text-sm text-gray-600'>
        ...
        {currentChest > 0 && currentChest <= p && (
          <span className='text-red-600 font-medium'>Đích đến: Rương {currentChest}</span>
        )}
      </Box>
    </Paper>
  );
